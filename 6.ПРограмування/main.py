...
class MainWindow(CTk):
   def __init__(self):
       ...
       self.chat_text = CTkTextbox(self.chat_frame, state = "disable")
       self.chat_text.pack(fill = "both", expand = True, padx = 5, pady = (5, 0))

       self.botton_frame = CTkFrame(self.chat_frame)
       self.botton_frame.pack(fill = "x", pady = 5)

       self.message_input = CTkEntry(self.botton_frame, placeholder_text='Введіть повідомлення:')
       self.message_input.pack(side = "left", fill = "x", expand = True, padx = (0, 5))

       self.send_button = CTkButton(self.botton_frame, text='▶️', width=40, command = self.send_message)
       ...
       try:
           ...
           self.add_message(f"Не вдалось підключитися: {e}")
           ...
           def handle_line(self, line):
               ...
               if msg_type == "TEXT":
                   if len(parts) >= 3:
                       author = parts[1]
                       message = parts[2]
                       self.add_messaage(f"{author}: {message}":)
               elif msg_type == "IMa":
   ...
   def save_username(self):
        ...
        if new_name:
           ...
           self.add_message(f"Нік змінено на {self.username}")
                ...
                self.add_message("Ім'я не може бути порожніл")
